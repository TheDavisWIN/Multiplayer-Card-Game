import { isHost, getState, onPlayerJoin, useMultiplayerState, usePlayersList } from "playroomkit";
...
const DECK_SELECTION = ["deckA", "deckB", "deckC"];
const DECK_CONTENTS = {
    deckA: [
        ...Array(16).fill({ card: "punch" }),
        ...Array(24).fill({ card: "grab" }),
        ...Array(8).fill({ card: "shield" }),
    ],
    deckB: [
        ...Array(16).fill({ card: "punch" }),
        ...Array(24).fill({ card: "grab" }),
...
export const GameEngineProvider = ({ children }) => {
    ...
    players.sort((a, b) => a.id.localeCompare(b.id)); // Sort players by id for consistency
    ...
    const shuffleDeck = () => {
        players.forEach((player) => {
            const deck = player.getState("deck") || [];
            for (let i = deck.length - 1; i > 0; i--) {
                const randomIndex = randInt(0, i);
                [deck[i], deck[randomIndex]] = [deck[randomIndex], deck[i]];
            }
            player.setState("deck", deck, true);
        });
    };

    const distributeDeckSelection = () => {
        players.forEach((player) => {
            player.setState("cards", [], true);
            const selectionDeck = [...DECK_SELECTION];
            player.setState("cards", selectionDeck, true);
            const cardsInHand = player.getState("cards");
            player.setState("selectedCard", -1, true);
        });
        setPhase("deckSelection", true);
    };
    ...
    const finalizeDecks = () => {
        players.forEach((player) => {
            ...
            if (selectedCard === -1) {
                const randomDeckKey = Object.keys(DECK_CONTENTS)[Math.floor(Math.random() * Object.keys(DECK_CONTENTS).length)];
                const chosenDeck = DECK_CONTENTS[randomDeckKey] || [];
                player.setState("deck", chosenDeck, true);
            } 
            else {
                const chosenDeck = DECK_CONTENTS[selectedCard] || [];
                player.setState("deck", chosenDeck, true);
            }
            player.setState("cards", [], true);
        });
        setPhase("setupBoard", true);
    };
    ...
    const startGame = () => {
        if (isHost()) {
            ...
            const randomPlayer = randInt(0, players.length - 1); // Choose a random player to start
    ...
    useEffect(() => {
        startGame();
        onPlayerJoin(startGame); // Restart game when a new player joins
    }, []);
